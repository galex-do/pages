...
На каждом сервере есть приложение-**агент** (он же shipper, он же listener), которое сканирует новые строки в файлах сервера, за которыми мы ему сказали следить. Агент отправляет собранные логи по сети к аггрегатору логов. Аггрегатор принимает логи, обрабатывает их, структурирует и пишет в базу. Визуализатор отправляет запросы к аггрегатору, чтобы достать из базы нужные логи и показать их в удобном интерфейсе. 
