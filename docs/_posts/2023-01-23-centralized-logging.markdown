...
Возьмем простой пример, для чего нам эти данные могут пригодиться. Обратимся к `/var/log/auth.log`, записи которого мы просматривали ранее.
...
Проанализировав выдачу поиска, мы можем сделать вывод, что кто-то пытается подобрать пользователя для подключения к серверу. Следовательно, стоит уделить внимание сетевой безопасности.
...
* (**-**) Аггрегация и поиск в Loki ограничены назначенными ему лейблами
...
2. Разобрались с концепцией централизованного логирования и зачем оно нужно
    ...
    * Видим динамику событий в логах и можем отслеживать аномалии
...
4. Попробовали настроить логирование с помощью Grafana и Loki
5. Познакомились с языком запросов LogQL

Знание, как организовать централизованное логирование, позволит вам упростить контроль событий в больших кластерах серверов, а также контейнерной инфраструктуре, такой как Docker и Kubernetes.
